glmdag = function(data_input, types_by_node = NULL, ordin_pred_as = "num", 
  ...
  modified_dag_result = 
    tryCatch(error = function(e) NA,
             push_dag(admm_results$Beta_est_by_lam, data_info))
  graph_est_by_lam = 
    tryCatch(error = function(e) NA, modified_dag_result$graph_est_by_lam)
  pushed_A_est_by_lam = 
    tryCatch(error = function(e) NA, modified_dag_result$A_est_by_lam)
  pushed_Beta_est_by_lam = 
    tryCatch(error = function(e) NA, modified_dag_result$Beta_est_by_lam)
  ...
  if (path_par == T){
    ...
    sel_tun = tryCatch(error = function(e) NA,
                       tun_sel_lkhd(pushed_A_est_by_lam, pushed_Beta_est_by_lam, data_info))
    
    fit = c(list(graph_est_by_lam = graph_est_by_lam, 
                 sel_tun = sel_tun,
                 pushed_A_est_by_lam = pushed_A_est_by_lam,
                 pushed_Beta_est_by_lam = pushed_Beta_est_by_lam), 
            admm_results, data_info)
