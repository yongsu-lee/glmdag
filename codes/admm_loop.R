admm_loop = function(W_update, Beta_update, W_init, Beta_init, white_coefs,
  ...
  Beta = Beta_init
  Mu = W_init - Beta_init
  W_est_by_lam = Beta_est_by_lam  <- list(0)
  ...
  for (ell in 1:n_lams){ # ell = 1
    ...
    if (warm_start == F) {
      W = W_init; Beta = Beta_init; Mu =  W_init - Beta_init 
    }
    ...
    while (k < max_iter ){
      ...
      #### W-update ----
      W_new <- tryCatch(error = function(e) NA,
                        W_update(W_init, Beta, Mu, rho, data_info, white_coefs, 
                                 inner_verbose, ver = ver, opt_method = opt_method) )
      
      if (is.na(W_new)[1] == T) {
        cat("+++++++++++++++++++++++++++++++++++++++++++++++++++++ \n")
        cat("Possible ill-conditioned happended. Move on next path \n")
        cat("+++++++++++++++++++++++++++++++++++++++++++++++++++++ \n")
        break
      }
      ...
      eps_pri[k]= sqrt(n_coefs) * abs_tol + 
        rel_tol * max(norm(W_new), norm(-Beta_new))
    ...
    ### If ill-conditioned cases move next path 
    if (is.na(W_new)[1] == T){
